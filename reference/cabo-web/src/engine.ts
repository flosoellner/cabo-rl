...
export const CARD_VALUES: number[] = (() => {
  ...
  for (let copy = 0; copy < 4; copy++) {
    for (let v = 1; v <= 12; v++) vals.push(v);
  }
...
export interface QTables {
  cabo: QTable;
  draw_source: QTable;
  swap_or_discard: QTable;
}
...
export interface AgentBrain {
  ...
  qtables: QTables;
  gamesPlayed: number;
  trajectory: Array<[keyof QTables, string, string]>;
}
...
export interface GameState {
  deck: DeckData;
  players: { human: Player; agent: Player };
...
export function defaultQTables(): QTables {
  return { cabo: {}, draw_source: {}, swap_or_discard: {} };
}

export function newAgentBrain(): AgentBrain {
  return { oppKnown: [], qtables: defaultQTables(), gamesPlayed: 0, trajectory: [] };
}

export function newDeck(): DeckData {
  const drawPile = [...CARD_VALUES];
  shuffle(drawPile);
  return { drawPile, discardPile: [] };
}

export function newGameState(agentBrain: AgentBrain): GameState {
  return {
    deck: newDeck(),
...
export function deckDraw(state: GameState): number {
  if (state.deck.drawPile.length === 0) deckReshuffle(state);
  if (state.deck.drawPile.length === 0) return 5; // extreme edge case
  return state.deck.drawPile.pop()!;
}

export function deckReshuffle(state: GameState): void {
...
export function handPop(state: GameState, who: Who, idx: number): number {
  const player = state.players[who];
  const val = player.hand.splice(idx, 1)[0];
  player.selfKnown.splice(idx, 1);
  if (who === "human") state.agentBrain.oppKnown.splice(idx, 1);
  return val;
}

export function handAppend(state: GameState, who: Who, val: number, known: boolean): void {
  const player = state.players[who];
  player.hand.push(val);
  player.selfKnown.push(known);
  if (who === "human") state.agentBrain.oppKnown.push(false);
}

export function handSet(state: GameState, who: Who, idx: number, val: number, known: boolean): void {
  const player = state.players[who];
  player.hand[idx] = val;
  player.selfKnown[idx] = known;
  if (who === "human") state.agentBrain.oppKnown[idx] = false;
}

export function performBlindSwap(state: GameState, humanIdx: number, agentIdx: number): void {
...
export function unseenPool(state: GameState): number[] {
  const counter = new Map<number, number>();
  for (const v of CARD_VALUES) counter.set(v, (counter.get(v) ?? 0) + 1);
  const agent = state.players.agent;
  const human = state.players.human;
  agent.selfKnown.forEach((known, i) => {
    if (known) counter.set(agent.hand[i], (counter.get(agent.hand[i]) ?? 0) - 1);
  });
  state.agentBrain.oppKnown.forEach((known, i) => {
    if (known && i < human.hand.length) counter.set(human.hand[i], (counter.get(human.hand[i]) ?? 0) - 1);
  });
  ...
  counter.forEach((cnt, val) => {
    for (let i = 0; i < cnt; i++) pool.push(val);
  });
  return pool;
}

export function expectedValue(pool: number[]): number {
  if (pool.length === 0) return 5.0;
  return pool.reduce((a, b) => a + b, 0) / pool.length;
}

export function estimateHandValue(hand: number[], knownFlags: boolean[], avgUnknown: number): number {
  let total = 0;
  for (let i = 0; i < hand.length; i++) total += knownFlags[i] ? hand[i] : avgUnknown;
  return total;
}

export function bucket(value: number, edges: number[]): number {
...
export function getEpsilon(brain: AgentBrain): number {
...
export function qGet(
  ...
  if (!(stateKey in table)) {
    ...
    for (const a of actions) entry[a] = defaults && a in defaults ? defaults[a] : 0.0;
...
export function qChoose(
  ...
  if (Math.random() < epsilon) return choice(actions);
  const bestVal = Math.max(...actions.map((a) => qvals[a]));
  const bestActions = actions.filter((a) => qvals[a] === bestVal);
  return choice(bestActions);
}

export function recordDecision(state: GameState, decisionName: keyof QTables, stateKey: string, action: string): void {
  state.agentBrain.trajectory.push([decisionName, stateKey, action]);
}

export function agentLearn(state: GameState, reward: number): void {
  ...
  for (const [decisionName, stateKey, action] of brain.trajectory) {
    ...
    if (!(action in qvals)) qvals[action] = 0.0;
...
export function agentDecideCabo(state: GameState): boolean {
  const brain = state.agentBrain;
...
export function agentDecideDrawSource(state: GameState): "pile" | "discard" {
  const brain = state.agentBrain;
  const discardPile = state.deck.discardPile;
  if (discardPile.length === 0) return "pile";
  ...
  return action as "pile" | "discard";
}

export function agentDecideSwapOrDiscard(state: GameState, card: number): "swap" | "discard" {
  const brain = state.agentBrain;
  const avg = expectedValue(unseenPool(state));
  const agent = state.players.agent;
  const ownEst = estimateHandValue(agent.hand, agent.selfKnown, avg);
...
export function agentChooseSwapPosition(state: GameState, card: number): number {
  const agent = state.players.agent;
  const avg = expectedValue(unseenPool(state));
  let bestPos = 0;
...
export function agentSwapIn(state: GameState, pos: number, card: number): void {
  const agent = state.players.agent;
...
export function agentUsePeekOwn(state: GameState): void {
  const agent = state.players.agent;
...
export function agentUseSpyOpp(state: GameState): void {
  const human = state.players.human;
  const brain = state.agentBrain;
  const unknownPositions: number[] = [];
  for (let i = 0; i < human.hand.length; i++) if (!brain.oppKnown[i]) unknownPositions.push(i);
  if (unknownPositions.length === 0) {
...
export function agentUseSwapBlind(state: GameState): void {
  const agent = state.players.agent;
  const human = state.players.human;
  const brain = state.agentBrain;
  const avg = expectedValue(unseenPool(state));
  ...
  const knownHumanPositions: number[] = [];
  for (let i = 0; i < human.hand.length; i++) if (brain.oppKnown[i]) knownHumanPositions.push(i);

  if (knownHumanPositions.length > 0) {
    let bestHumanPos = knownHumanPositions[0];
    for (const i of knownHumanPositions) if (human.hand[i] < human.hand[bestHumanPos]) bestHumanPos = i;
...
export function agentTryMatchDrawn(state: GameState, card: number): boolean {
...
export function agentTurn(state: GameState, final: boolean): boolean {
  if (!final) {
  ...
  if (fromDiscard) {
    ...
    if (action === "swap") {
      ...
      if (POWER_PEEK_OWN.has(card)) agentUsePeekOwn(state);
      else if (POWER_SPY_OPP.has(card)) agentUseSpyOpp(state);
      else if (POWER_SWAP_BLIND.has(card)) agentUseSwapBlind(state);
    }
  }
  return false;
}
...
// ---------------------------------------------------------------------------
// Human actions (each is a single discrete step - the UI layer sequences them)
// ---------------------------------------------------------------------------

export type PowerKind = "peek" | "spy" | "swap" | null;

export function humanDrawFromPile(state: GameState): number {
  return deckDraw(state);
}

export function humanTakeDiscardCard(state: GameState): number {
  return state.deck.discardPile.pop()!;
}

export function humanPlaceCardInto(state: GameState, card: number, pos: number): void {
  const human = state.players.human;
...
export function humanDiscardDrawn(state: GameState, card: number): PowerKind {
  state.deck.discardPile.push(card);
  logMsg(state, `You discard the ${card} face-up.`);
  if (POWER_PEEK_OWN.has(card)) return "peek";
  if (POWER_SPY_OPP.has(card)) return "spy";
  if (POWER_SWAP_BLIND.has(card)) return "swap";
  return null;
}

export function humanUsePeekOwn(state: GameState, pos: number): number {
  const human = state.players.human;
...
export function humanUseSpyOpp(state: GameState, pos: number): number {
...
export function humanUseSwapBlind(state: GameState, hpos: number, apos: number): void {
...
export interface PlaceDrawnResult {
  success: boolean;
  positions: number[];
  values: number[];
}
...
export function humanPlaceDrawnCard(state: GameState, card: number, positions: number[]): PlaceDrawnResult {
...
export function dealNewRound(state: GameState): void {
  state.deck = newDeck();
  for (const who of ["human", "agent"] as const) {
    const p = state.players[who];
    p.hand = [];
    for (let i = 0; i < 4; i++) p.hand.push(state.deck.drawPile.pop()!);
...
export function isKamikaze(hand: number[]): boolean {
  const sorted = [...hand].sort((a, b) => a - b);
  return sorted.length === 4 && sorted[0] === 12 && sorted[1] === 12 && sorted[2] === 13 && sorted[3] === 13;
}

export function resolveRound(state: GameState, caboCaller: Who | null): Record<Who, number> {
  const human = state.players.human;
  const agent = state.players.agent;
  ...
  const roundPoints: Record<Who, number> = { human: 0, agent: 0 };
  ...
  if (humanKamikaze || agentKamikaze) {
    ...
    if (caboCaller === "agent") roundPoints.agent += CABO_PENALTY;
  } else if (agentSum < humanSum) {
    roundPoints.agent = 0;
    roundPoints.human = humanSum;
    if (caboCaller === "human") roundPoints.human += CABO_PENALTY;
  } else {
    if (caboCaller === "human") {
      roundPoints.human = 0;
      roundPoints.agent = agentSum;
...
export function applyRoundScores(state: GameState, roundPoints: Record<Who, number>): Who | null {
  for (const who of ["human", "agent"] as const) state.players[who].totalScore += roundPoints[who];
  for (const who of ["human", "agent"] as const) {
...
export function determineNextStarter(roundPoints: Record<Who, number>, caboCaller: Who | null, previousStarter: Who): Who {
  const zeroPlayers = (["human", "agent"] as const).filter((w) => roundPoints[w] === 0);
  if (zeroPlayers.length === 1) return zeroPlayers[0];
  if (zeroPlayers.length === 2 && caboCaller) return caboCaller;
