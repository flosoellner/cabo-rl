...
export function unseenDistribution(state, who) {
    const counts = new Map();
    for (const v of E.CARD_VALUES)
        counts.set(v, (counts.get(v) ?? 0) + 1);
    const player = state.players[who];
    player.hand.forEach((v, i) => {
        if (player.selfKnown[i])
            counts.set(v, (counts.get(v) ?? 0) - 1);
    });
    if (who === "agent") {
        ...
        state.agentBrain.oppKnown.forEach((known, i) => {
            if (known && i < human.hand.length)
                counts.set(human.hand[i], (counts.get(human.hand[i]) ?? 0) - 1);
        });
    }
    for (const v of state.deck.discardPile)
        counts.set(v, (counts.get(v) ?? 0) - 1);
...
function encodeHand(hand, belief, remembered) {
...
export function encodeState(state, who, deckSize, isFinalTurn, drawnCard = null) {
    ...
    const oppEnc = encodeHand(opp.hand, belief, (i) => who === "agent" && state.agentBrain.oppKnown[i] ? opp.hand[i] : null);
    const discardTop = state.deck.discardPile.length > 0
