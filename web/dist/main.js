...
function renderHands() {
    ...
    <div class="piles">
      ...
      <div class="pile ${flow.current === "human" && ui.phase === "choose_action" && state.deck.discardPile.length > 0 ? "pile--clickable" : ""}"
           ${flow.current === "human" && ui.phase === "choose_action" && state.deck.discardPile.length > 0 ? 'data-action="take-discard"' : ""}>
        <div class="pile-card pile-card--discard">${state.deck.discardPile.length > 0 ? state.deck.discardPile[state.deck.discardPile.length - 1] : "-"}</div>
...
root.addEventListener("click", (e) => {
    ...
    switch (action) {
        ...
        case "own-card":
            if (pos !== null)
                app.onOwnCardClick(pos);
            break;
        case "agent-card":
            if (pos !== null)
                app.onAgentCardClick(pos);
            break;
